    ...
    <>
      <div className="">
        <div className="flex items-center h-screen gap-3 max-w-7xl mx-auto py-5">
          ...
          <CustomerInfo totalPrice={totalPrice} />
        </div>
      </div>
    </>
  );
...
const CartProducts = () => {
  const { cart, removeToCart, handleQuantity } = useContext(StoreContext);
  return (
    <>
      {cart.map((product, index) => (
        <div
          ...
          key={index}
        >
          <div className=" bg-gray-50 h-20 w-[16%] flex items-center justify-center">
            <img
              src={product.image}
              alt=""
          ...
          <div className="text-sm flex items-center w-[20%]">
            <div className=" border w-16 h-8 rounded-3xl flex justify-center items-center gap-1 m-auto font-semibold">
              <p>
                {product.kg} <span className="font-normal">K.G</span>
              </p>
            </div>
          </div>
